...
1. `then` must return a promise [[4.2](#notes)].
    ...
    1. If either `onFulfilled` or `onRejected` returns a thenable (call it `returnedThenable`), `promise2` must attempt to adopt the state of `returnedThenable` under the assumption of it being a promise [[4.3](#notes)]:
